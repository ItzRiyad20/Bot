...
def create_embed(title, description="", color=COLOR_PRIMARY):
    ...
    embed.set_footer(
        text=f"Made by ItzRiyad20 • v{BOT_VERSION} • {datetime.now().strftime('%H:%M:%S')}",
...
async def my_vps(ctx):
    ...
    embed.set_footer(text=f"Made by ItzRiyad20 • VPS Control Panel")
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        # Example: ItzRiyad20-vps-1, alexuser-vps-2, btw-infinite-vps-3
...
"""
                ...
                # Get all network interfaces - with timeout to prevent hanging
                                # Start Pinggy tunnel after reinstall
                pinggy_raw = await start_pinggy_tunnel(self.container_name, self.node_id)
                pinggy_host, pinggy_port = parse_pinggy_url(pinggy_raw) if pinggy_raw else (None, None)

                # Store tunnel info
                target_vps['pinggy_host'] = pinggy_host
                target_vps['pinggy_port'] = pinggy_port

                # Get internal network IPs
                try:
                    networks = await asyncio.wait_for(
                ...
                # Build SSH info showing BOTH tunnel and internal IPs
                if pinggy_host and pinggy_port:
                    ssh_access_info = (
                        f"**🌐 Pinggy Tunnel (Recommended):**\n"
                        f"`ssh root@{pinggy_host} -p {pinggy_port}`\n\n"
                    )
                    if networks:
                        ssh_access_info += "**🖥️ Internal Network IPs:**\n"
                        for interface, ip in sorted(networks.items()):
                            ssh_access_info += f"└─ **{interface}:** `ssh root@{ip}`\n"
                        ssh_access_info += "\n"
                        ssh_access_info += (
                        ...
                        f"🔄 If tunnel disconnects, use `{PREFIX}manage` → **🔄 Reconnect Tunnel**."
                    )
                else:
                    if networks:
                        ssh_access_info = "**🖥️ Available Connection Points:**\n"
                        ...
                        ssh_access_info = (
                            ...
                            f"**Username:** `root`\n"
                            f"**Password:** `{new_password}`\n\n"
                            f"📡 Network initializing, check in a few seconds.\n"
                            f"Use `{PREFIX}manage` → **🔄 Reconnect Tunnel** for SSH access.\n\n"
                            f"⚠️ Save this password securely!"
                        )
...
class ManageView(discord.ui.View):
    ...
    async def create_vps_embed(self, index):
        ...
        # Pinggy tunnel info
        pinggy_host = vps.get('pinggy_host')
        pinggy_port = vps.get('pinggy_port')
        if pinggy_host and pinggy_port:
            tunnel_info = (
                f"**SSH Command:**\n"
                f"`ssh root@{pinggy_host} -p {pinggy_port}`\n"
                f"**Host:** `{pinggy_host}`\n"
                f"**Port:** `{pinggy_port}`\n"
                f"**Status:** 🟢 Tunnel Active\n"
                f"*(Click 🔄 Reconnect Tunnel if disconnected)*"
            )
        else:
            tunnel_info = (
                f"**Status:** 🔴 No tunnel active\n"
                f"*(Click 🔄 Reconnect Tunnel to start)*"
            )
        add_field(embed, "🌐 Pinggy SSH Tunnel", tunnel_info, False)
    ...
    def add_action_buttons(self):
        ...
        self.add_item(reconnect_button)   # ← নতুন
        ...
        if action == 'start':
            ...
            try:
                ...
                if node and node['is_local']:
                    ...
                    _, _, rc = await run_shell(
                        f"{LXC_CMD} exec {cn} -- bash -c 'apt-get update -qq -y && apt-get install -y tmate'",
                        timeout=120
                    )
                    if rc != 0:
                    ...
                    await run_shell(f"{LXC_CMD} exec {cn} -- tmate -S {sock} new-session -d -s {session_name}", timeout=15)
                    await asyncio.sleep(3)
                    stdout, _, _ = await run_shell(
                        f"{LXC_CMD} exec {cn} -- tmate -S {sock} display -p '#{{tmate_ssh}}'",
                        timeout=10
                    )
            ...
            try:
                ...
                if node and node['is_local']:
                    ...
                    check_out, _, check_rc = await shell(
                        f"{LXC_CMD} exec {cn} -- bash -c 'command -v sshx || ls /root/.local/bin/sshx 2>/dev/null || ls /usr/local/bin/sshx 2>/dev/null'",
                        timeout=10
                    )
            ...
            if suspended:
                await interaction.followup.send(
                    embed=create_error_embed("Access Denied", "Cannot start tunnel on a suspended VPS."),
                    ephemeral=True)
                return
            current_status = target_vps.get('status', 'stopped')
            if current_status != 'running':
                await interaction.followup.send(
                    embed=create_error_embed("VPS Not Running", "Please start the VPS first."),
                    ephemeral=True)
                return
            await interaction.followup.send(
                embed=create_info_embed("🔄 Reconnecting Tunnel", "Restarting Pinggy tunnel, please wait..."),
                ephemeral=True)
            try:
                pinggy_raw = await start_pinggy_tunnel(container_name, node_id)
                pinggy_host, pinggy_port = parse_pinggy_url(pinggy_raw) if pinggy_raw else (None, None)
                if pinggy_host and pinggy_port:
                    # Store tunnel info
                    target_vps['pinggy_host'] = pinggy_host
                    target_vps['pinggy_port'] = pinggy_port
                    vps_data[self.owner_id][actual_idx]['pinggy_host'] = pinggy_host
                    vps_data[self.owner_id][actual_idx]['pinggy_port'] = pinggy_port
                    save_vps_data_immediate()
                    tunnel_embed = create_success_embed("✅ Tunnel Connected!")
                    tunnel_embed.add_field(
                        name="🔐 SSH Command",
                        value=f"`ssh root@{pinggy_host} -p {pinggy_port}`",
                        inline=False)
                    tunnel_embed.add_field(
                        name="👤 Username",
                        value="`root`",
                        inline=True)
                    stored_pass = target_vps.get('root_password', 'Check your DM')
                    tunnel_embed.add_field(
                        name="🔑 Password",
                        value=f"`{stored_pass}`",
                        inline=True)
                    try:
                        await interaction.user.send(embed=tunnel_embed)
                        await interaction.followup.send(
                            embed=create_success_embed("✅ Sent!", "SSH credentials have been sent to your DM."),
                            ephemeral=True)
                    except discord.Forbidden:
                        await interaction.followup.send(embed=tunnel_embed, ephemeral=True)
                else:
                    await interaction.followup.send(
                        embed=create_error_embed("Tunnel Failed", "Pinggy tunnel could not start. Please check if your VPS has internet access."),
                        ephemeral=True)
            except Exception as e:
                await interaction.followup.send(
...
async def vps_list(ctx, node_id: int = 1):
    ...
    if vps_info:
        ...
        for idx, chunk in enumerate(chunks[1:], 2):
            ...
            page_embed.set_footer(text=f"Made by ItzRiyad20 • {len(vps_info)} VPS shown")
            await ctx.send(embed=page_embed)
    else:
        add_field(embed, "📋 **VPS List**", "No deployments yet. Launch one! 🚀", False)

    embed.set_footer(text=f"Made by ItzRiyad20 • Total: {len(vps_info)} VPS")
    await ctx.send(embed=embed)
...
async def system_status(ctx):
    ...
    embed.set_footer(text=f"Made by ItzRiyad20 • System Status • Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                    icon_url=BOT_ICON_URL)
...
async def user_info(ctx, user: discord.Member):
    ...
    embed.set_footer(text="Made by ItzRiyad20 • User Resource Dashboard")
    embed.timestamp = ctx.message.created_at
...
async def server_stats(ctx):
    ...
    embed.set_footer(text="Made by ItzRiyad20 • Real-Time Monitoring")
    embed.timestamp = ctx.message.created_at
...
async def vps_info(ctx, container_name: str = None):
    if not container_name:
        ...
        for idx, chunk in enumerate(chunks, 1):
            ...
            embed.set_footer(text=f"Made by ItzRiyad20 • VPS Information System")
            await ctx.send(embed=embed)
    else:
        found_vps = None
        found_user = None
        ...
        embed.set_footer(text=f"Made by ItzRiyad20 • VPS Information System • Container: {container_name}")
        await ctx.send(embed=embed)
...
async def vps_password(ctx, container_name: str = None):
    ...
    if not container_name:
        ...
        for idx, chunk in enumerate(chunks, 1):
            ...
            embed.set_footer(text=f"Made by ItzRiyad20 • Password Management")
            await ctx.send(embed=embed)
    else:
        ...
        embed.set_footer(text=f"Made by ItzRiyad20 • Password Information")
        await ctx.send(embed=embed)
...
async def quick_help(ctx):
    ...
    embed.set_footer(text=f"Made by ItzRiyad20 • Use {PREFIX}help for complete command list")
    await ctx.send(embed=embed)
...
async def help_search(ctx, *, search_term: str = None):
    ...
    embed.set_footer(text=f"Made by ItzRiyad20 • Use {PREFIX}help for complete list")
    await ctx.send(embed=embed)    
...
def parse_pinggy_url(raw_url: str):
    """Parse pinggy tunnel URL into (host, port) tuple"""
    # Format: tcp://xxxx.free.pinggy.io:PORT
    try:
        raw = raw_url.strip()
...
async def start_pinggy_tunnel(container_name: str, node_id: int) -> Optional[str]:
    """Start Pinggy tunnel inside VPS and return the tunnel URL"""
    try:
        # Kill any existing pinggy/ssh tunnel
        try:
            await execute_lxc(container_name,
                f"exec {container_name} -- bash -c "
                f"'pkill -f \"tcp@free.pinggy.io\" || true; pkill -f pinggy || true; sleep 1'",
                node_id=node_id)
        except Exception:
            pass

        # Install openssh-server + client if not present
        await execute_lxc(container_name,
            f"exec {container_name} -- bash -c "
            f"'apt-get install -y openssh-server openssh-client 2>/dev/null || true'",
            node_id=node_id, timeout=120)

        # Clear old log
        try:
            await execute_lxc(container_name,
                f"exec {container_name} -- bash -c 'rm -f /tmp/pinggy.log'",
                node_id=node_id, timeout=10)
        except Exception:
            pass

        # Start tunnel in background, save output to log
        tunnel_cmd = (
            "nohup ssh -p 443 "
            "-o StrictHostKeyChecking=no "
            "-o ServerAliveInterval=30 "
            "-o ExitOnForwardFailure=yes "
            "-R0:127.0.0.1:22 tcp@free.pinggy.io "
            "> /tmp/pinggy.log 2>&1 &"
        )
        await execute_lxc(container_name,
        ...
        # Wait for tunnel URL to appear in log (max 30 sec)
        for i in range(15):
            await asyncio.sleep(2)
            try:
                out = await execute_lxc(container_name,
                    f"exec {container_name} -- bash -c 'cat /tmp/pinggy.log 2>/dev/null'",
                    node_id=node_id, timeout=10)
                if isinstance(out, str):
                    # Log full output for debugging
                    logger.info(f"[Pinggy] Attempt {i+1} log for {container_name}:\n{out}")
                    import re
                    match = re.search(r'tcp://([^\s:]+):(\d+)', out)
                    if match:
                        url = f"tcp://{match.group(1)}:{match.group(2)}"
                        logger.info(f"[Pinggy] Found URL: {url}")
                        return url
            except Exception as e:
                logger.warning(f"[Pinggy] Read error attempt {i+1}: {e}")

        # Log final output before giving up
        try:
            final_out = await execute_lxc(container_name,
                f"exec {container_name} -- bash -c 'cat /tmp/pinggy.log 2>/dev/null'",
                node_id=node_id, timeout=10)
            logger.error(f"[Pinggy] Final log (no URL found) for {container_name}:\n{final_out}")
        except Exception:
            pass

        return None
    except Exception as e:
        logger.error(f"Pinggy tunnel failed for {container_name}: {e}")
        return None
...
def parse_pinggy_url(raw_url: str):
    """Parse pinggy tunnel URL into (host, port) tuple"""
    try:
        # Find tcp:// anywhere in the string
        import re
        match = re.search(r'tcp://([^\s:]+):(\d+)', raw_url)
        if match:
    ...
    try:
        ...
        if pinggy_host and pinggy_port:
            ssh_info = (
                ...
                f"⚠️ If tunnel disconnects, use `{PREFIX}manage` → **🔄 Reconnect Tunnel**."
            )
        else:
            ssh_info = (
                f"⚠️ Pinggy tunnel could not start.\n"
                f"Use `{PREFIX}manage` → **🔄 Reconnect Tunnel** to start it.\n\n"
                f"**Username:** `root`\n"
                f"**Password:** `{root_password}`\n\n"
                f"🔒 Save this password!"
            )
